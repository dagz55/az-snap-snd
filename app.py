...
async def process_vm(resource_id, vm_name, resource_group, disk_id, chg_number, timestamp, log_file, user_id):
    ...
    # Include the user ID as a tag in the snapshot
    stdout, stderr, returncode = await run_az_command(
        f"az snapshot create --name {snapshot_name} --resource-group {resource_group} --source {disk_id} --tags CreatedByUserId={user_id}"
    )
...
async def main(host_file, chg_number, ttl_duration):
    global TTL_DURATION
    ...
    vm_list, error = extract_vm_info(host_file)
    if error:
        return {"error": error}
